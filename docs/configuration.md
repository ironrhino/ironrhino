...
##配置文件说明
1. 应用启动需要用到的配置文件`classpath:ironrhino.properties`, 在里面指定应用的名称和应用的基础包名, 这些参数在应用启动之前就需要用到并且是跟环境无关, 示例如下: 
``` 
app.name=demo
app.basePackage=com.demo
```
2. 可在不同环境下覆盖配置项的配置文件, 主要是给spring容器用, 所以命名也是 `applicationContext.properties`, 一共有四个配置文件, 都是可选的, 后面文件的配置项覆盖前面文件里面的配置项.
...
这几个文件最终合并成的Properties给spring作为placeholder用, java方式和xml方式分别如下: 
```java
