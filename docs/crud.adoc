...
对于多选, 只需要将字段类型设置为数组或者集合类型, 和数据库映射还需要技巧:
[source,java]
.Product.java
----
...
----
在字段上增加 `@Transient` , 另外增加一对 `getTagsAsString/setTagsAsString` 方法, 在`getTagsAsString`增加一些注解.
多选同样也可以用枚举类型实现.

有单选的情况下, 可以以多tab方式展示列表,
比如 http://localhost:8080/product/tabs ,
