...
public class JsonPointResultMatchersTest {
	...
	public void test() throws Exception {
		...
		given(response.getContentAsString()).willReturn(
				"{\"i\":100, \"pi\":3.1, \"str\":\"str\", \"arr\":[1, 3.1, \"str\"], \"map\":{\"i\":100, \"pi\":3.1, \"str\":\"str\"}}");
		jsonPoint("/i").value(100).match(mvcResult);
		jsonPoint("/pi").value(3.1F).match(mvcResult);
		jsonPoint("/pi").value(3.1D).match(mvcResult);
		jsonPoint("/str").value("str").match(mvcResult);
		jsonPoint("/arr").value(new Object[] { 1, 3.1, "str" }).match(mvcResult);
		jsonPoint("/arr").value(Arrays.asList(1, 3.1, "str")).match(mvcResult);
		jsonPoint("/arr/1").value(3.1D).match(mvcResult);
