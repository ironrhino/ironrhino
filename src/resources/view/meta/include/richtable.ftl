...
<#list columns as name,config>
	...
	<@rttbodytd entity=entity entity@index=entity?index value=value celleditable=celleditable&&config['cellEdit']?? template=config['template']! cellDynamicAttributes=config['dynamicAttributes'] dynamicAttributes=dynamicAttributes/>
</#list>
<@rttbodytrend entity=entity entity@index=entity?index showActionColumn=showActionColumn buttons=actionColumnButtons editable=!readonly viewable=viewable entityReadonly=entityReadonly/>
</#list>
...
<#macro rttbodytd value,entity,entity@index,celleditable=true,template='',cellDynamicAttributes='',dynamicAttributes...>
...
<#macro rttbodytrend entity entity@index showActionColumn=true buttons='' editable=true viewable=false entityReadonly=false inputWindowOptions='' viewWindowOptions=''>
<#if showActionColumn>
<td class="action">
...
<form method="post" class="ajax view form-horizontal groupable query ignore-blank" data-columns="${gridColumns}">
	...
	<@s.submit label=getText('query') class="btn-primary">
		<@s.param name="after"> <@s.reset/></@s.param>
	</@s.submit>
</form>
</#if>
</#macro>