...
<span class="input-append">
    <input type="text" maxlength="9" name="resultPage.pageNo" value="${resultPage.pageNo}" class="inputPage integer positive" title="${action.getText('currentpage')}"/><span class="add-on totalPage"><span class="divider">/</span><strong title="${action.getText('totalpage')}">${resultPage.totalPage}</strong></span>
</span>
<#if showPageSize>
...
<#if !request.requestURI?ends_with('/pick') && totalResults gt 0 && totalResults lte (csvMaxRows!10000) && action.csv??>
