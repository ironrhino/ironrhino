	...
	<#if !(entity.new&&readonly)>
		...
		<#elseif config.type=='enum'>
			<#if !config.multiple||config.cssClasses?seq_contains('chosen')>
				...
				<@s.select disabled=readonly multiple=config.multiple id=id label=label name=name class=config.cssClass list=config.listOptions?eval listKey=config.listKey listValue=config.listValue headerKey="" headerValue="" dynamicAttributes=dynamicAttributes/>
			<#else>
				<#if readonly><#if value?has_content><#list value as en><@s.hidden id="" name=name value=en.name()/></#list></#if></#if>
				<@s.checkboxlist disabled=readonly id=id label=label name=name class=config.cssClass list=config.listOptions?eval listKey=config.listKey listValue=config.listValue dynamicAttributes=dynamicAttributes/>
			</#if>
				...
				<#list 0..(collection?has_content?then(collection?size-1,0)) as index>
					...
					<tr<#if folded> class="template"</#if>>
						...
						<#elseif config.type=='enum'>
							<#if !config.multiple||config.cssClasses?seq_contains('chosen')>
								<#if readonly><@s.hidden name=name value="${(value.name())!}"/></#if>
								<@s.select disabled=readonly multiple=config.multiple id="" theme="simple" name=name class=config.cssClass list=config.listOptions?eval listKey=config.listKey listValue=config.listValue headerKey="" headerValue="" dynamicAttributes=dynamicAttributes/>
							<#else>
								<#if readonly><#if value?has_content><#list value as en><@s.hidden id="" name=name value=en.name()/></#list></#if></#if>
								<@s.checkboxlist disabled=readonly id="" theme="simple" name=name class=config.cssClass list=config.listOptions?eval listKey=config.listKey listValue=config.listValue dynamicAttributes=dynamicAttributes/>
							</#if>
						<#elseif config.type=='select'>
							<#if readonly><@s.hidden name=name/></#if>
