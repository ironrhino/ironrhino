		    ...
		    <div id="module_${key?index}_${apiModule?index}" class="accordion-body collapse<#if currentModule> in</#if>">
		      <div class="accordion-inner">
		        <ul class="nav nav-list">
					...
					<li<#if currentModule && api?has_content && api==apiDoc.name> class="active"</#if>><a href="${actionBaseUrl}?<#if _category?has_content>category=${_category?url}&</#if>module=${apiModule.name?url}&api=${apiDoc.name?url}<#if version?has_content>&version=${version}</#if>" class="ajax view history" data-replacement="apidoc">${apiDoc.name}</a></li>
					</#list>
				</ul>
		      </div>
		    </div>
		...
		<#if apiDoc??>
			...
			<table class="table">
				<tbody>
					...
					<#if apiDoc.pathVariables?has_content><tr><td>URI变量</td><td><@listFields fields=apiDoc.pathVariables/></td></tr></#if>
