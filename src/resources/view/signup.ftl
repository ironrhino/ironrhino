...
<div class="row">
	...
	<@s.form method="post" action="${actionBaseUrl}" class="ajax focus form-horizontal well">
		...
		<@s.password label=getText('password') name="password" class="required span2 sha"/>
		<@s.password label=getText('confirmPassword') name="confirmPassword" class="required span2 sha"/>
