...
<@s.form action="${actionBaseUrl}/password" method="post" class="form-horizontal ajax focus reset">
	...
	<@s.password label=getText('currentPassword') name="currentPassword" class="required input-pattern sha" readonly=userProfileReadonly/>
	</#if>
	<@s.password label=getText('password') name="password" class="required input-pattern sha" readonly=userProfileReadonly/>
	<@s.password label=getText('confirmPassword') name="confirmPassword" class="required repeat input-pattern submit sha" data\-repeatwith="password" readonly=userProfileReadonly/>
