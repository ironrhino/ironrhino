...
public class EchoHandler extends TextWebSocketHandler {
	...
	private final Set<WebSocketSession> sessions = Collections
			.newSetFromMap(new ConcurrentHashMap<WebSocketSession, Boolean>());
	...
	public void broadcast(String message, Predicate<UserDetails> p) {
		for (WebSocketSession s : sessions)
			if (s.isOpen())
				try {
					if (p.evaluate(userDetailsService.loadUserByUsername(s
	...
	public void handleTextMessage(WebSocketSession session, TextMessage message)
		...
		String text = new StringBuilder(username).append(" send : ")
				.append(message.getPayload()).toString();
