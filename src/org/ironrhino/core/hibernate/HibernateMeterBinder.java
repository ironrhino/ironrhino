...
public class HibernateMeterBinder implements MeterBinder {
	...
	@NonNullApi
	@NonNullFields
	static class HibernateMetrics implements MeterBinder {
		...
		public void bindTo(MeterRegistry registry) {
			...
			// AWKWARD: getSecondLevelCacheRegionNames is the only way to retrieve a list of
			// names
			...
			Arrays.stream(statistics.getSecondLevelCacheRegionNames()).map(s -> {
				if (cacheFactoryPrefix != null) {
					return s.replaceAll(cacheFactoryPrefix + ".", "");
				}
				return s;
			}).filter(this::hasDomainDataRegionStatistics).forEach(regionName -> {
