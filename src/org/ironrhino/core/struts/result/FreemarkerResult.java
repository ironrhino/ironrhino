 ...
 * </li>
 *
 * <li>a classpath resuorce. eg
 * <code>/com/company/web/views/home.ftl</code></li>
 *
 * </ul>
 *
 ...
 * 
 * <li><b>writeIfCompleted</b> - false by default, write to stream only if there
 * isn't any error processing the template. Setting
 * template_exception_handler=rethrow in freemarker.properties will have the
 * same effect.</li>
 *
 * </ul>
 *
...
public class FreemarkerResult extends StrutsResultSupport {
	 ...
	 * substitutions
	 * <p/>
	 * this method obtains the freemarker configuration and the object wrapper from
			...
			try {
				...
				if (isWriteIfCompleted()
					...
					if (isTopTemplate = (parentCharArrayWriter == null)) {
						...
						// set it in the request because when the "action" tag is used a new VS and
	...
	/**
	 * This method is called from {@link #doExecute(String, ActionInvocation)} to
	 * obtain the FreeMarker configuration object that this result will use for
	 * template loading. This is a hook that allows you to custom-configure the
	 * configuration object in a subclass, or to fetch it from an IoC container.
	 * <p/>
	...
	/**
	 * This method is called from {@link #doExecute(String, ActionInvocation)} to
	 * obtain the FreeMarker object wrapper object that this result will use for
	 * adapting objects into template models. This is a hook that allows you to
	 * custom-configure the wrapper object in a subclass.
	 * <p/>
	 ...
	 * implementation simply returns the locale setting of the action (assuming the
	 * action implements {@link LocaleProvider}) or, if the action does not the
	 * configuration's locale is returned. Override this method to provide different
	 * behaviour,
	 */
	 ...
	 * generic hook you might use in subclasses to perform a specific action before
	 * the template is processed. By default does nothing. A typical action to
	 * perform here is to inject application-specific objects into the model root
	 *
